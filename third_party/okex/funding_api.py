...
class FundingAPI(Client):
    ...
    def funds_transfer(self, ccy, amt, _from, to, _type='0', sub_acct=None, inst_id=None, to_inst_id=None):
        params = {'ccy': ccy, 'amt': amt, 'from': _from, 'to': to, 'type': _type, 'subAcct': sub_acct, 'instId': inst_id,
                  'toInstId': to_inst_id}
        return self._request_with_params(POST, FUNDS_TRANSFER, params)

    # Withdrawal
    def coin_withdraw(self, ccy, amt, dest, to_addr, pwd, fee):  # noqa
        params = {'ccy': ccy, 'amt': amt, 'dest': dest, 'toAddr': to_addr, 'pwd': pwd, 'fee': fee}  # noqa
    ...
    def purchase_redempt(self, ccy, amt, side):  # noqa
        params = {'ccy': ccy, 'amt': amt, 'side': side}
        return self._request_with_params(POST, PURCHASE_REDEMPT, params)

    # Get Withdrawal History
    def get_bills(self, ccy=None, _type=None, after=None, before=None, limit=None):
        params = {'ccy': ccy, 'type': _type, 'after': after, 'before': before, 'limit': limit}
